      ...
      <div className="max-w-3xl mx-auto">
        <div className="max-w-lg mx-auto flex-1">
          <FlipMove>
            {/* {messages?.docs?.map((message) => (
              ...
              time = {message?.time?.toDate().getTime()}
              />
            ))}
             */}
        ...
        <form className="flex items-center justify-between space-x-1 w-96 mx-auto sticky bottom-2 z-10">
          ...
          <button
            disabled={!input}
            onClick={sendMessage}
            className="bg-green-400 text-sm font-bold text-white p-3 rounded-lg hover:scale-95 transition-all duration-200 ease-in-out disabled:bg-gray-200 disabled:cursor-not-allowed"
          >
            Send
          </button>
        </form>
